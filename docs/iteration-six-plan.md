...
| 4 | Regression checks, synthetic two-round service loop, browser review, production deployment and read-only verification | Unit and Worker suites, prior workspace flows, export/reload, failure handling, public build verified | Local acceptance complete; production gated by CI |
| 5 | Expand native exchange import into workspace; Polis/Proposals/Sensemaker round trips; compare analysis versions | Versioned real exports and destination support verified separately for each edge | Follow-on |
