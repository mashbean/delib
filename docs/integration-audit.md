...
| Pocket Check（口袋理解關卡） | Hand 1–10 sourced single-choice questions, a pass mark and a next link to pocket-check; Delib returns the quiz URL, the public results URL and a one-time host link. The next link is returned by the Worker only on a pass; every attempt is recorded; results give pass rate, per-question accuracy and the most common wrong choice. | None. Delib stores neither the questions nor the attempts or the token. | MIT single-Worker implementation of the comprehension-check idea (SwarmCheck has no source); no model calls; the next link is an invitation, not access control. | Available with managed create. |
| Pocket Proposals（口袋提案） | Hand a prompt and description to pocket-proposals; Delib returns the space URL, the public results URL and a one-time host link. Participants propose, endorse, respond and amend (full text + rationale, line diff); only the author accepts an amendment, which bumps the version. Exports Polis seeds and tttc.csv. | None. Delib stores nothing. | MIT single-Worker implementation of the proposal/amendment/endorsement shape from Decidim and Go Vocal (AGPL, no code reused); no thresholds, phases or votes. | Available with managed create. |
| Pocket Argument（口袋論證） | Hand one contestable claim to pocket-argument; Delib returns the debate URL, the public results URL and a one-time host link. Participants add supporting or attacking arguments to bounded depth and vote whether each holds (never their own); strength is a smoothed vote margin plus child feedback. Exports arguments.csv, tree.json and tttc.csv. | None. Delib stores nothing. | MIT single-Worker independent design (SwarmCheck has no source); heuristic strength formula documented in the README; no fact-checking. | Available with managed create. |
| Pocket Maple（口袋公聽） | Hand a Legislative Yuan bill number (or a manual agenda item) to pocket-maple; Delib returns the testify URL, the public archive URL and a one-time host link. The Worker snapshots the bill (proposer, status, reason, progress) from the token-free ly.govapi.tw API at creation; each device leaves one stance + summary + testimony, revisable and withdrawable until the deadline; the archive is public and filterable by stance; exports testimonies.csv, archive.json and tttc.csv. | None. Delib stores neither the testimonies nor the token. | MIT single-Worker Taiwan adaptation of MAPLE's shape (no code reused); identity is not verified and every testimony says so. | Available with managed create. |
