...
| 6 | Moral Graph Elicitation | **Pocket Values**: value-elicitation dialogue → moral graph | Method is published; code has no license, so clean-room only. Shares the dialogue infrastructure with Pocket Harmonica, so it follows it. | done 2026-09-05 (pocket-values.mashbean.workers.dev; dialogue → values card → pairwise "wiser than" → Bradley–Terry moral graph; no card dedup yet) |
