...
describe("public contracts", () => {
  ...
  it("ships a source-linked registry with 32 unique tools", async () => {
    ...
    expect(registry.tools).toHaveLength(32);
    expect(new Set(registry.tools.map((tool) => tool.id)).size).toBe(32);
  ...
  it("accounts for every catalog tool in the direct-integration audit", async () => {
    ...
    expect(auditedIds).toHaveLength(32);
    expect(new Set(auditedIds).size).toBe(32);
    ...
    expect(audit.integrations.filter((item) => item.activation === "managed-create")).toHaveLength(6);
  });
  ...
  it("publishes one source and hosting decision for every tool", async () => {
    ...
    expect(hosting.tools).toHaveLength(32);
