...
export function createJourneyScene(canvas,{onSelect,labels,reducedMotion=false}) {
 ...
 catch { canvas.hidden=true;const fallback=document.createElementNS('http://www.w3.org/2000/svg','svg');fallback.setAttribute('viewBox','0 0 1000 400');fallback.setAttribute('role','img');fallback.setAttribute('aria-label','八步驟審議流程 / Eight deliberation stages');fallback.classList.add('scene-fallback');fallback.innerHTML='<path d="M60 270 C200 80 300 350 470 190 S730 50 940 170" fill="none" stroke="currentColor"/>'+labels().map((name,i)=>'<g><circle cx="'+(60+i*125)+'" cy="'+(i%2?170:270)+'" r="22" fill="none" stroke="currentColor"/><text x="'+(60+i*125)+'" y="'+(i%2?175:275)+'" fill="currentColor" text-anchor="middle">'+(i+1)+'</text></g>').join('');canvas.after(fallback);return {update(){},reset(){},setTilt(){},zoom(){}}; }
 ...
 const scene=new THREE.Scene();scene.fog=new THREE.FogExp2(0x0a0e14,.009);
 const camera=new THREE.PerspectiveCamera(34,1,.1,120);camera.position.set(1,13,24);
 const controls=new OrbitControls(camera,canvas);controls.target.set(0,.5,0);controls.enableDamping=false;controls.dampingFactor=.08;controls.enablePan=false;controls.enableZoom=false;controls.minPolarAngle=.25;controls.maxPolarAngle=Math.PI*.52;controls.rotateSpeed=.6;if(compact){controls.enableRotate=false;canvas.style.touchAction='pan-y';}
 ...
 for(let i=0;i<8;i++){
   ...
   const pedestal=mesh(new THREE.CylinderGeometry(.78,.93,.12,6),colors[i],group);pedestal.position.y=-.6;pedestal.material.transparent=true;pedestal.material.opacity=.25;
   const rim=new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CylinderGeometry(.78,.93,.12,6)),new THREE.LineBasicMaterial({color:colors[i],transparent:true,opacity:.5}));rim.position.y=-.6;group.add(rim);
 ...
 function resize(){const r=canvas.getBoundingClientRect();if(!r.width||!r.height)return;renderer.setSize(r.width,r.height,false);camera.aspect=r.width/r.height;camera.zoom=Math.min(.78,camera.aspect/1.6);camera.setViewOffset(r.width,r.height,-r.width*.14,r.height*.21,r.width,r.height);camera.updateProjectionMatrix();render();}
 function render(){if(!visible||document.hidden)return;controls.update();renderer.render(scene,camera);}
 function frame(now){raf=0;if(!visible||document.hidden)return;const elapsed=now-last;if(elapsed>45)slowFrames++;else slowFrames=Math.max(0,slowFrames-1);if(slowFrames>35&&quality>.8){quality=Math.max(.8,quality-.25);renderer.setPixelRatio(quality);slowFrames=0;}const dt=Math.max(0,Math.min(elapsed/1000,.04))||0;last=now;if(!paused){clock+=dt;nodes.forEach((n,i)=>{n.form.rotation.y=clock*.13+i*.2;n.form.position.y=Math.sin(clock*.7+i)*.05;});packets.forEach((p,i)=>{const edge=i%7,t=(clock*.17+Math.floor(i/7)/6)%1;p.position.copy(curves[edge].getPoint(t));p.visible=true;p.scale.setScalar(edge<step?1.5:1);});}render();if(!paused)raf=requestAnimationFrame(frame);}
 ...
 const api={
  update(opts={}) {const old=step;step=opts.step??step;paused=opts.paused??paused;mode=opts.mode??mode;peopleData=opts.people??peopleData;
   ...
   packets.forEach((p,i)=>{p.position.copy(curves[i%7].getPoint(Math.floor(i/7)/6));p.visible=true;});render();start();
  },
  zoom(factor){camera.zoom=THREE.MathUtils.clamp(camera.zoom*factor,.25,2.4);camera.updateProjectionMatrix();render();},
  reset(){camera.zoom=Math.min(.78,camera.aspect/1.6);camera.updateProjectionMatrix();if(window.gsap&&!paused)gsap.to(camera.position,{x:1,y:13,z:24,duration:1,ease:'power2.inOut',onUpdate:render});else camera.position.set(1,13,24);controls.target.set(0,.5,0);render();},
  setTilt(value){camera.position.y=Number(value)/2;render();}
 };
 function applyTheme(){lightTheme=document.documentElement.dataset.theme==='light';const bg=lightTheme?0xf4f6ef:0x090d13;renderer.setClearColor(bg,1);scene.fog.color.setHex(bg);(Array.isArray(grid.material)?grid.material:[grid.material]).forEach(m=>{m.color.setHex(lightTheme?0x8ca994:0x253c3a);m.transparent=true;m.opacity=lightTheme?.16:.65;});dust.material.color.setHex(lightTheme?0x355642:0xb2c4c9);packets.forEach(p=>p.material.color.setHex(lightTheme?0x3c681c:0xd8ffa8));people.forEach(p=>p.children.forEach(m=>m.material.color.setHex(lightTheme?0x962b63:0xff91d5)));language='';setLabels();canvas.dataset.sceneTheme=lightTheme?'light':'dark';render();}
 window.addEventListener('delib:themechange',applyTheme);applyTheme();
 window.addEventListener('pagehide',e=>{cancelAnimationFrame(raf);raf=0;if(!e.persisted){observer.disconnect();resizeObserver.disconnect();controls.dispose();renderer.dispose();}});window.addEventListener('pageshow',e=>{if(e.persisted){resize();start();}});resize();start();return api;
}
