...
try{
 ...
 await page.locator('[data-pane=story]').click();await page.locator('#story [data-choice="0"]').click();assert.match(await page.locator('#story .story-outcome').innerText(),/Round two/);assert.equal(await page.locator('#demo-rounds [data-round="1"]').getAttribute('aria-pressed'),'true');
 ...
 await page.locator('#confirm-send [name=consent]').check();await page.locator('#confirm-send button').click();await page.locator('[data-action=read-form]').click();await page.locator('.work-record').first().waitFor({state:'attached'});if(!await page.locator('.work-record').first().isVisible())await page.locator('.record-browser>summary').click();assert.equal(await page.locator('.work-record').count(),2);
 ...
 await page.locator('.work-record').first().waitFor({state:'attached'});if(!await page.locator('.work-record').first().isVisible())await page.locator('.record-browser>summary').click();await page.locator('.work-record').first().click();await page.locator('.record-detail .review-tools summary').click();await page.locator('#review-record [name=reviewer]').fill('Fictional facilitator');await page.locator('#review-record button').click();
 await page.locator('[data-step="2"]').click();await page.locator('#create-reply [name=speaker]').fill('Fictional facilitator');await page.locator('#create-reply button').click();assert.match(await page.locator('.send-data').innerText(),/Keep the accessible bay/);await page.locator('#confirm-send [name=consent]').check();await page.locator('#confirm-send button').click();await page.locator('[data-action=read-reply]').click();await page.locator('.work-record').first().waitFor({state:'attached'});
 await page.locator('[data-tab=voices]').click();await page.locator('.work-record').first().click();await page.locator('.voice-trail article').nth(2).waitFor();assert.equal(await page.locator('.voice-trail article').count(),3);const trailDownload=page.waitForEvent('download');await page.locator('[data-action=export-trace]').click();await (await trailDownload).saveAs('/private/tmp/delib-third-voice.json');
 ...
 await page.goto(`${base}/voice?lang=en`);await page.locator('#receipt-file').setInputFiles('/private/tmp/delib-third-voice.json');await page.locator('.voice-trail article').nth(2).waitFor();assert.equal(await page.locator('.voice-trail article').count(),3);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
