...
- Pocket Polis 資料工作台多了「讀取綜整」：把 Pocket Polis 用 Workers AI
  Gemma（或無模型的統計摘要）整理出的概述、跨群共同點與張力，挑選後放進
  公開成果頁；成果頁把它標成「工具整理」，附模型名稱、產生時間、是否過期與
  引用的陳述編號，和主辦者解讀分開，並在資料邊界加上限制聲明。
- Power Ranker 短期收件室的主辦者可以「停止收件」；已收到的結果仍可看到
  到期為止，參與者送出時會看到清楚的說明。
- Markdown 匯出多一段工具整理。

### 營運者需要知道

- 新增 `ReceiptIndex` Durable Object：只記錄短網址的 slug、種類與時間；
  `GET /api/receipts` 帶 `X-Receipt-Operator` 可列出目前存在的公開成果，
  方便下架與濫用檢視。到期或刪除時自動移除。
- 新增 `GET /api/integrations/pocket-polis/synthesis?conversation=<id>`：
  只代理設定的 Pocket Polis 主機，只回傳白名單欄位，計入每 IP 寫入限流
  （因為上游可能因此啟動一次模型工作），快取 60 秒。
- 新增 `POST /api/integrations/power-ranker/rooms/:id/close`（需管理權杖）；
  收件室資料表加上 `accepting` 欄位，舊房間第一次讀取時自動升級。
- 公開成果收據的伺服器驗證改為逐項核算：成對合計、判斷總數、每個選項的
  比較次數、名次唯一、涵蓋率；Pocket Polis 收據的陳述編號不得重複、
  選句回應總數不得超過整輪票數；時間一律要求 ISO 8601，日期必須真的存在。
- pilot 會把綜整節錄放進收據並多做三項檢查（模型與時間、引用陳述存在、
  限制聲明）。
- `delib-pocket-polis-receipt/v1` schema 新增選填的 `toolSynthesis`。

### 資料邊界

- 工具整理層只含 Pocket Polis 公開成果頁本來就看得到的綜整文字與陳述
  編號；不含模型看過的逐筆投票或匿名代碼。是否放進成果頁由主辦者勾選。
